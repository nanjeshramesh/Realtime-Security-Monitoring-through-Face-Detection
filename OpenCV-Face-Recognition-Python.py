...
#So if you have not read it, I encourage you to do so to understand how face
#detection works and its Python coding. 

# ### Import Required Modules

# Before starting the actual coding we need to import the required modules for coding.
#So let's import them first. 
# 
# - **cv2:** is _OpenCV_ module for Python which we will use for face detection
#and face recognition.
# - **os:** We will use this Python module to read our training directories and file names.
# - **numpy:** We will use this module to convert Python lists to numpy arrays as
#OpenCV face recognizers accept numpy arrays.
...
# 
# So our training data consists of total 2 persons with 12 images of each person.
#All training data is inside _`training-data`_ folder. _`training-data`_ folder contains
#one folder for each person and **each folder is named with format `sLabel (e.g. s1, s2)`
#where label is actually the integer label assigned to that person**. For example folder
#named s1 means that this folder contains images for person 1. The directory structure
#tree for training data is as follows:
# 
# ```
# training-data
# |-------------- s1
...
# ```
# 
# The _`test-data`_ folder contains images that we will use to test our face
#recognizer after it has been successfully trained.

# As OpenCV face recognizer accepts labels as integers so we need to define a
#mapping between integer labels and persons actual names so below I am defining
#a mapping of persons integer labels and their respective names. 
# 
# **Note:** As we have not assigned `label 0` to any person so **the mapping for label 0
#is empty**. 

# In[2]:

#there is no label 0 in our training data so subject name for index/label 0 is empty
subjects = ["", "manusudhan", "gagan"]


# ### Prepare training data

# You may be wondering why data preparation, right? Well, OpenCV face recognizer
#accepts data in a specific format. It accepts two vectors, one vector is of faces of
#all the persons and the second vector is of integer labels for each face so that when
...
#So for example, in this tutorial we have folder names: `s1, s2`. 
# 2. For each subject, extract label number. **Do you remember that our folders have a
#special naming convention?** Folder names follow the format `sLabel` where `Label` is an
#integer representing the label we have assigned to that subject. So for example, folder name
#`s1` means that the subject has label 1, s2 means subject label is 2 and so on.
...
# 4. Add each face to faces vector with corresponding subject label (extracted in above step)
#added to labels vector. 
# 
# **[There should be a visualization for above steps here]**

# Did you read my last article on [face detection]
#(https://www.superdatascience.com/opencv-face-detection/)? No?
#Then you better do so right now because to detect faces, I am going to use the code from my previous article on [face detection](https://www.superdatascience.com/opencv-face-detection/). So if you have not read it, I encourage you to do so to understand how face detection works and its coding. Below is the same code.
